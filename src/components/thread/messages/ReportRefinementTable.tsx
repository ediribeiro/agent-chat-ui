...
import { ArrowUp, ArrowDown, ArrowDownUp, ArrowRight } from "lucide-react";

interface ReportRefinementItem {
  Id: string;
  Risco: string;
  Fonte: string;
...
interface ReportRefinementTableProps {
  data: ReportRefinementItem[];
  prevStats?: Record<string, number>;
}

export default function ReportRefinementTable({ data, prevStats }: ReportRefinementTableProps) {
  ...
  // Compact statistics UI with comparison
  return (
    <Card className="w-full max-w-4xl mx-auto my-6 border-blue-200 shadow-md overflow-x-auto">
      <CardHeader className="bg-blue-50 border-b">
        <CardTitle className="text-blue-700 text-lg font-semibold">Report Refinement</CardTitle>
        <div className="flex flex-wrap gap-2 mt-2">
          {Object.entries(stats).map(([cat, count]) => {
            const prev = prevStats?.[cat] ?? 0;
            let icon = null;
            let color = "";
            if (count > prev) { icon = <ArrowUp className="inline w-3 h-3 text-green-600" />; color = "text-green-700"; }
            else if (count < prev) { icon = <ArrowDown className="inline w-3 h-3 text-red-600" />; color = "text-red-700"; }
            else { icon = <ArrowRight className="inline w-3 h-3 text-blue-600" />; color = "text-blue-600"; }
            return (
              <div key={cat} className="flex flex-col items-center bg-blue-100 rounded px-2 py-1 min-w-[80px]">
                <span className={`text-xs font-medium truncate max-w-[70px] ${color}`}>{cat}</span>
                <span className={`text-base font-bold ${color}`}>{count} {icon}</span>
              </div>
            );
          })}
        </div>
